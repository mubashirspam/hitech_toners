...
export default function QuotationTemplate({
    ...
    return (
        <div id={id} className="w-[1024px] font-sans bg-white scale-[calc(718/1024)] origin-top-left p-4">
            <div className="grid grid-cols-3 gap-4">
                <div className="space-y-1">
                    <h2 className="text-[#440873] font-bold text-lg">Shams Al Ghuroub Trading Est.</h2>
                    <p className="text-black text-base font-semibold">WHOLESALE & RETAIL - SALES & SERVICE</p>
                    <p className="text-sm">MECCA-SAUDI ARABIA</p>
                    <p className="text-sm">VAT No. : 302134781700003</p>
                </div>

                <div className="flex items-start justify-center">
                    <img src={logo} alt="shams logo" className="w-full max-w-32 object-contain bg-blend-multiply" />
                </div>

                <div dir="rtl" className="space-y-1">
                    <h2 className="text-[#440873] font-bold text-xl">مؤسسة شمس الغروب التجارية</h2>
                    <p className="text-black text-base font-semibold">البيع بالجملة والتجزئة-المبيعات والخدمات</p>
            ...
            <div>
                <div className="bg-[#440873] text-white font-bold flex">
                    ...
                    <div className="py-1.5 px-2.5 w-32 shrink-0 text-right">UNIT PRICE</div>
                ...
                <div>
                    {items.map((item, index) => (
                        <div key={index} className="border-b divide-x divide-border flex">
                            <div className="py-1.5 px-3 w-16 shrink-0 text-center">{index + 1}</div>
                            <div className="py-1.5 px-3 flex-1">{item.description}</div>
                            <div className="py-1.5 px-3 w-32 shrink-0 text-right">{Number(item.quantity || 0)}</div>
                            <div className="py-1.5 px-3 w-32 shrink-0 text-right">{Number(item.unitPrice || 0)}</div>
            ...
            <div className="flex justify-between my-10">
                <div className="w-full max-w-xs">
                    ...
                    <p className="text-sm">3 MONTHS WARRANTY FOR SERVICE & CHANGED PARTS FROM THE DATE OF INVOICE</p>
                </div>

                <div className="w-full max-w-xs divide-y divide-border border-y">
                    <div className="grid grid-cols-2">
                        <div className="py-1.5 px-2.5 font-bold">TOTAL</div>
                        <div className="text-right py-1.5 px-2.5 flex items-center justify-end">{total.toFixed(2)}</div>
                    </div>
                    ...
                    <div className="grid grid-cols-2">
                        <div className="bg-[#440873] text-white py-1.5 px-2.5 font-bold">GRAND TOTAL</div>
